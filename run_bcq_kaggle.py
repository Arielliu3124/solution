...
from dbrl_1.models import BCQ
from dbrl_1.network import VAE, Perturbator, Critic
from dbrl_1.trainer import train_model
from dbrl_1.utils.params import count_vars, init_param_bcq
import pdb

def parse_args():
    parser = argparse.ArgumentParser(description="run_bcq_kaggle")
    ...
    parser.add_argument("--user_dict", type=str,
                        default="user_dict.npy")
    parser.add_argument("--users_histroy_len", type=str,
                        default="users_histroy_len.npy")
    parser.add_argument("--n_epochs", type=int, default=100)
    parser.add_argument("--hist_num", type=int, default=10,
                        help="num of history items to consider")
    parser.add_argument("--n_rec", type=int, default=9,
...
if __name__ == "__main__":
    ...
    '''
    with open(os.path.join("bigdata2021-rl-recsys", args.user_dict), "rb") as f:
        user_dict = np.load(f ,allow_pickle=True)
        '''
    with open(os.path.join("bigdata2021-rl-recsys", args.users_histroy_len), "rb") as f:
        users_histroy_len = np.load(f , allow_pickle=True)
    ...
    pert_lr = args.lr
    critic_lr = args.lr
    weight_decay_gen = args.weight_decay
    weight_decay_pert = args.weight_decay
    ...
    neg_sample = 1
    one_hour = int(60 * 60)
    reward_map = {"1": 1.,"2":2.0,"3":3.0,"4":4.0}
    #reward_map = None
    '''
    columns = ["user", "item", "label", "time", "sex", "age", "pur_power",
               "category", "shop", "brand"]'''
    '''
    columns = ["user", "item", "time", "label", "user_p0", "user_p1", "user_p2","user_p3","user_p4","user_p5",
                            "user_p6","user_p7","user_p8","user_p9", "price", "location", "item_v0","item_v1","item_v2","item_v3","item_v4"]
    '''
    ...
    train_loader, eval_loader = build_dataloader(n_users,n_items,hist_num,train_user_consumed,
        test_user_consumed,
        batch_size,
        sess_mode=sess_mode,
    ...
    generator = VAE(
        input_dim, action_dim, action_dim * 2, hidden_size, user_embeddings,
        item_embeddings, None, pad_val, 1, device
    ).to(device)
    perturbator = Perturbator(input_dim, action_dim, hidden_size).to(device)
    critic1 = Critic(input_dim, action_dim, hidden_size).to(device)
    critic2 = Critic(input_dim, action_dim, hidden_size).to(device)
    init_param_bcq(generator, perturbator, critic1, critic2)

    gen_optim = Adam(
        generator.parameters(), gen_lr, weight_decay=weight_decay_gen
    )
    pert_optim = Adam(
        perturbator.parameters(), pert_lr, weight_decay=weight_decay_pert
    )
    critic_params = itertools.chain(critic1.parameters(), critic2.parameters())
    critic_optim = Adam(
        critic_params, critic_lr, weight_decay=weight_decay_critic
    )
    ...
    var_counts = tuple(count_vars(module) for module in [
        generator, perturbator, critic1, critic2
    ])
    print(f'Number of parameters: generator: {var_counts[0]}, '
          f' perturbator: {var_counts[1]},  critic1: {var_counts[2]}, '
          f' critic2: {var_counts[3]}')
    ...
    torch.save({"generator": generator.state_dict(),
                "perturbator": perturbator.state_dict()},
               "bigdata2021-rl-recsys/model_bcq.pt")

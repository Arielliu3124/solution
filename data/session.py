...
from dbrl_1.utils.misc import compute_returns
import pdb
...
def pad_session(hist_len, hist_num, hist_items, pad_val):
    ...
    #print('sess_len=' + str(sess_len))
    #pdb.set_trace()
    session_first = np.full((sess_len, hist_num + 1), pad_val, dtype=np.int64)
    for i in range(sess_len):
        offset = i + 2
        session_first[i, -offset:] = hist_items[:offset]                  #x x x x 1 2     x x x 1 2 3
...
def build_session(
    ...
    for u in range(n_users):
        ...
        hist_len = len(items)
        #print('hist_num='+ str(hist_num),'hist_len='+ str(hist_len))
        #pdb.set_trace()
